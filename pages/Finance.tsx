      ...
      {printInvoice && (
        <div className="fixed inset-0 z-[100] bg-white overflow-y-auto">
           <div className="max-w-4xl mx-auto p-8 bg-white min-h-screen relative">
              <button onClick={() => setPrintInvoice(null)} className="absolute top-4 right-4 print:hidden p-2 bg-gray-100 rounded-full hover:bg-gray-200">
                <X size={24} />
              </button>
              ...
              <div className="flex justify-between items-start mb-12 border-b border-gray-200 pb-8">
                 <div>
                    ...
                    <p className="text-gray-500 font-mono">{printInvoice.number}</p>
                 </div>
                 <div className="text-right rtl:text-left">
                    <h2 className="text-xl font-bold text-red-600">Lais Qatar</h2>
                    <p className="text-sm text-gray-500">Fire & Safety Experts</p>
                    <p className="text-sm text-gray-500">Building 42, Street 840, Doha</p>
                    <p className="text-sm text-gray-500">TRN: 123456789</p>
                 </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-8 mb-12">
                 <div>
                    ...
                    <p className="font-bold text-gray-900 text-lg">{printInvoice.customerId}</p>
                    <p className="text-gray-600">Doha, Qatar</p>
                 </div>
                 <div className="grid grid-cols-2 gap-4">
                    <div>
                       <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-1">Issue Date</h3>
                       <p className="font-medium text-gray-900">{printInvoice.date}</p>
                    </div>
                    <div>
                       <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-1">Due Date</h3>
                       <p className="font-medium text-gray-900">{printInvoice.dueDate}</p>
                    </div>
                    <div>
                       <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-1">Status</h3>
                       <p className="font-medium text-gray-900">{printInvoice.status}</p>
                    </div>
                 </div>
              </div>
              ...
              <table className="w-full mb-12">
                 <thead>
                    <tr className="bg-gray-50 border-y border-gray-200">
                       <th className="py-3 px-4 text-left rtl:text-right font-bold text-gray-700">Description</th>
                       <th className="py-3 px-4 text-right rtl:text-left font-bold text-gray-700 w-32">Amount</th>
                    </tr>
                 </thead>
                 <tbody className="divide-y divide-gray-100">
                    {printInvoice.items.map((item, i) => (
                       <tr key={i}>
                          ...
                          <td className="py-4 px-4 text-right rtl:text-left font-mono">{formatCurrency(item.amount)}</td>
                       </tr>
                    ))}
                 </tbody>
              </table>
              ...
              <div className="flex justify-end mb-16">
                 <div className="w-64 space-y-3">
                    <div className="flex justify-between text-gray-600">
                       <span>Subtotal</span>
                       <span>{formatCurrency(printInvoice.totalAmount)}</span>
                    </div>
                    <div className="flex justify-between text-gray-600">
                       <span>Tax (0%)</span>
                       <span>QAR 0.00</span>
              ...
              <div className="text-center text-sm text-gray-400 border-t border-gray-100 pt-8">
                 <p>Thank you for your business. Please make payments via bank transfer to QNB Account #1234-5678-9012.</p>
              </div>
           </div>
        </div>
      )}
                    ...
                    <tr key={inv.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-right rtl:text-left">
                        <div className="flex justify-end rtl:justify-start space-x-2 rtl:space-x-reverse">
                          <button onClick={() => handlePrint(inv)} className="text-gray-400 hover:text-red-600 p-1 hover:bg-red-50 rounded" title="Print Invoice">
                            <Printer size={18} />
                          </button>
